...
def select_folder_cli(default_path=None):
    """Select target folder using CLI input."""
    if default_path:
        folder_path = Path(default_path).expanduser().resolve()
        if folder_path.is_dir():
            return str(folder_path)
        print(f"⚠️  Provided path is not a directory: {default_path}")

    while True:
        choice = input("Enter MP3 directory path (or 'q' to exit): ").strip()
        ...
        print(f"⚠️  Invalid directory: {choice}")
...
def read_mp3_metadata(file_path):
    ...
    try:
        ...
        metadata = {
            ...
            or any(k.startswith("APIC") for k in tags.keys()),
        }
...
def search_musicbrainz(query, search_type="recording"):
    ...
    try:
        musicbrainzngs.set_useragent(
            "MP3Tagger", "1.0", "https://github.com/user/mp3tagger"
        )
...
def select_cover_art_file(default_path=None):
    """Select cover art image path through CLI."""
    if default_path:
        img_path = Path(default_path).expanduser().resolve()
        if img_path.is_file():
            with open(img_path, "rb") as f:
                return f.read()
        print(f"⚠️  Provided cover art path is not a file: {default_path}")

    while True:
        choice = input("Enter cover art image path (or blank to skip): ").strip()
        if not choice:
            return None

        img_path = Path(choice).expanduser().resolve()
        if img_path.is_file():
            with open(img_path, "rb") as f:
                return f.read()

        print(f"⚠️  Invalid image file: {choice}")
...
def display_current_metadata(files, root_dir):
    """Display current metadata for all MP3 files."""
    print(f"\n{'='*80}")
    print("CURRENT MP3 METADATA")
    print(f"{'='*80}")

    for idx, file_path in enumerate(files, 1):
        relative_path = file_path.relative_to(root_dir)
        print(f"\n[{idx:3d}] {relative_path}")

        metadata = read_mp3_metadata(file_path)

        if metadata:
            print(f"      Title:  {metadata.get('title', '(none)')}")
            print(f"      Artist: {metadata.get('artist', '(none)')}")
            print(f"      Album:  {metadata.get('album', '(none)')}")
            print(f"      Year:   {metadata.get('year', '(none)')}")
            print(f"      Genre:  {metadata.get('genre', '(none)')}")
            print(f"      Track:  {metadata.get('track', '(none)')}")
            print(f"      Cover:  {'Yes' if metadata.get('has_cover') else 'No'}")

            # Show bitrate and duration
            bitrate_kbps = (
                metadata.get("bitrate", 0) // 1000 if metadata.get("bitrate") else 0
            )
            duration_min = int(metadata.get("duration", 0) // 60)
            duration_sec = int(metadata.get("duration", 0) % 60)
            print(
                f"      Info:   {bitrate_kbps} kbps, {duration_min}:{duration_sec:02d}"
            )
        else:
...
def bulk_edit_fields(selected_files):
    ...
    for field_key, field_label in fields:
        ...
        response = (
            input(
                f"  → Apply '{value}' to all {len(selected_files)} files now? (y/n): "
            )
            .strip()
            .lower()
        )
    ...
    if choice == "1":
        ...
        while True:
            ...
            try:
                ...
                if not selected_indices.issubset(valid_indices):
                    print(
                        f"❌ Invalid file numbers. Must be between 1 and {len(selected_files)}"
                    )
                    continue

                # Get selected files
                files_to_update = [
                    selected_files[i - 1] for i in sorted(selected_indices)
                ]
                break
...
def edit_metadata_interactive(
    file_path, existing_metadata, bulk_values=None, bulk_cover_art=None, use_online=True
):
    """
    ...
    if choice == "1":
        ...
        return edit_metadata_interactive(
            file_path, existing_metadata, bulk_values, bulk_cover_art
        )
...
def lookup_and_edit(
    ...
    default_query = (
        ...
        or existing_metadata.get("album", "")
        or file_path.stem
    )
    ...
    # Online lookup may be disabled
    if not use_online:
        print("\n⚠️  Online lookup disabled; switching to manual input")
        return manual_edit(existing_metadata, bulk_values, bulk_cover_art)
    ...
    if not results and DISCOGS_AVAILABLE:
        ...
        try:
            ...
            if 1 <= idx <= len(results):
                ...
                new_metadata = {
                    ...
                    "genre": bulk_values.get("genre")
                    or existing_metadata.get("genre", ""),
...
def manual_edit(existing_metadata, bulk_values=None, bulk_cover_art=None):
    ...
    if bulk_cover_art is None:
        response = (
            input(
                f"\nCurrent cover art: {cover_status}. Add/replace cover art? (y/n): "
            )
            .strip()
            .lower()
        )
    ...
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            for file_path, metadata in zip(file_list, metadata_list):
                ...
                f.write(
                    f'"{filename}","{artist}","{album}","{title}","{track}","{year}","{genre}",{bitrate},{duration:.2f}\n'
                )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MP3 metadata tagger (CLI, works on Manjaro Linux)"
    )
    parser.add_argument(
        "path", nargs="?", default=None, help="Directory containing MP3 files"
    )
    parser.add_argument(
        "--no-online",
        action="store_true",
        help="Disable MusicBrainz/Discogs online lookup",
    )
    parser.add_argument(
        "--apply-all",
        action="store_true",
        help="Automatically apply default workflow without prompts",
    )
...
def main(args):
    ...
    print("=" * 80)
    print("  MP3 METADATA TAGGER for Manjaro/Linux (CLI)")
    print("=" * 80)
    ...
    if not use_online:
        print("\n⚠️  Online lookup disabled (MusicBrainz/Discogs will be skipped)")

    # Select folder (CLI)
    print("\n📂 Target folder selection")
    selected_folder = select_folder_cli(args.path)
    ...
    # Ask if user wants to see current metadata
    print("\n" + "=" * 80)
    print("METADATA PREVIEW")
    print("=" * 80)
    response = (
        input("\nDo you want to see current metadata for all files? (y/n): ")
        .strip()
        .lower()
    )
    if response in ["y", "yes"]:
        display_current_metadata(mp3_files, selected_folder)
    ...
    for idx, file_path in enumerate(selected_files, 1):
        ...
        # Interactive editing
        new_data = edit_metadata_interactive(
            file_path,
            existing_metadata,
            bulk_values,
            file_bulk_cover,
        ...
        success, error = write_mp3_metadata(
            file_path, new_data["metadata"], new_data.get("cover_art")
        )
